...
test('router devuelve null para URL que no matchea', async () => {
  ...
  const r = await handleAppAuth(makeReq('GET', 'https://sivocloud.dev/api/other/x'), makeEnv(), url)
  assert.equal(r, null)
})

test('router devuelve null para boxId con formato inválido', async () => {
  const url = new URL('https://sivocloud.dev/api/app-auth/INVALID/verify')
  const r = await handleAppAuth(makeReq('GET', 'https://sivocloud.dev/api/app-auth/INVALID/verify'), makeEnv(), url)
  assert.equal(r, null)
})

test('método incorrecto en /me → 405', async () => {
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/me`)
  const r = await handleAppAuth(makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/me`), makeEnv(), url)
  assert.equal(r.status, 405)
})

test('método incorrecto en /consume → 405', async () => {
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/consume`)
  const r = await handleAppAuth(makeReq('GET', `https://sivocloud.dev/api/app-auth/${BOX_ID}/consume`), makeEnv(), url)
  assert.equal(r.status, 405)
})
...
test('POST /request — email inválido → respuesta genérica (200)', async () => {
  ...
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/request`)
  const req = makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/request`, {}, { email: 'no-email' })
...
test('POST /request — body no es JSON → 400 invalid_body', async () => {
  mockControlPlaneNoSession()
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/request`)
  const req = makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/request`, {}, null)
...
test('POST /consume — body sin token → 400 missing_token', async () => {
  mockControlPlaneNoSession()
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/consume`)
  const req = makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/consume`, {}, {})
  const r = await handleAppAuth(req, makeEnv(), url)
  assert.equal(r.status, 400)
  const body = await r.json()
...
test('GET /me sin sesión plataforma → 401', async () => {
  mockControlPlaneNoSession()
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/me`)
  const req = makeReq('GET', `https://sivocloud.dev/api/app-auth/${BOX_ID}/me`)
  const r = await handleAppAuth(req, makeEnv(), url)
...
test('GET /admin/users sin sesión plataforma → 401', async () => {
  mockControlPlaneNoSession()
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users`)
  const req = makeReq('GET', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users`)
  const r = await handleAppAuth(req, makeEnv(), url)
...
test('POST /admin/users con sesión plataforma pero sin DB → 404 (gate antes de DB)', async () => {
  ...
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users`)
  const req = makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users`,
    {}, { email: 'x@example.com' })
  const r = await handleAppAuth(req, makeEnv(), url)
...
test('POST /admin/users método GET no permitido', async () => {
  mockControlPlaneAsPlatformUserNoBox({ role: 'editor' })
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users`)
  const req = makeReq('DELETE', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users`)
  const r = await handleAppAuth(req, makeEnv(), url)
  assert.equal(r.status, 405)
})
...
test('POST /admin/users/{id}/disable método GET no permitido', async () => {
  mockControlPlaneAsPlatformUserNoBox({ role: 'editor' })
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users/au_abc/disable`)
  const req = makeReq('GET', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users/au_abc/disable`)
  const r = await handleAppAuth(req, makeEnv(), url)
  assert.equal(r.status, 405)
})
...
test('DELETE /admin/users/{id} método POST no permitido', async () => {
  mockControlPlaneAsPlatformUserNoBox({ role: 'editor' })
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users/au_abc`)
  const req = makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users/au_abc`)
  const r = await handleAppAuth(req, makeEnv(), url)
  assert.equal(r.status, 405)
})
...
test('POST /admin/settings método GET sí permitido (gate antes de DB)', async () => {
  mockControlPlaneAsPlatformUserNoBox({ role: 'editor' })
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/settings`)
  const req = makeReq('POST', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/settings`,
    {}, { signup_mode: 'invalid' })
  const r = await handleAppAuth(req, makeEnv(), url)
...
test('anexo H4 — GET /admin/settings con rol viewer → 403 (no ve signup_mode)', async () => {
  mockControlPlaneAsPlatformUserWithBox({ role: 'viewer' })
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/settings`)
  const req = makeReq('GET', `https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/settings`)
  const r = await handleAppAuth(req, makeEnv(), url)
  assert.equal(r.status, 403)
  const body = await r.json()
  assert.equal(body.error, 'forbidden')
...
test('router matchea /api/app-auth/{boxId}/request|verify|consume|me|logout', async () => {
  for (const op of ['request', 'verify', 'consume', 'me', 'logout']) {
    const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/${op}`)
...
test('router matchea /admin/users y /admin/users/{id}/{action}', async () => {
  for (const path of [
    ...
    const url = new URL(`https://sivocloud.dev${path}`)
    const r = await handleAppAuth(makeReq('GET', url.toString()), makeEnv(), url)
...
test('router rechaza userId con formato inválido', async () => {
  ...
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users/INVALID!/disable`)
...
test('router rechaza action inválida en /users/{id}/{action}', async () => {
  ...
  const url = new URL(`https://sivocloud.dev/api/app-auth/${BOX_ID}/admin/users/au_abc/destroy`)
  const r = await handleAppAuth(makeReq('POST', url.toString()), makeEnv(), url)
  assert.equal(r, null)
})