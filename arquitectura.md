...
|---|---|---|
| `htmlbox-control-plane` | `controlplane.htmlbox.app` | Registry de tenants/workspaces/boxes, auth, billing, **aprovisionamiento de la Turso DB de cada box**, panel admin (Alpine) |
| `htmlbox-portal` | `portal.htmlbox.app` | Tenant Portal (Alpine): crear/subir boxes, gestionar datos y automatizaciones, compartir, permisos |
| `htmlbox-runtime` | `*.htmlbox.app` | Sirve el HTML de cada box (desde R2), Data API del box, ejecuta flows (flow-engine), webhooks de ingesta, endpoint MCP |
...
cookie `sid` HttpOnly con `Domain=.htmlbox.app` para compartir sesión entre
...
```
https://{tenant}.htmlbox.app/{boxSlug}/api/ingest/{tabla}
```
...
| Tipo | URL | Acceso |
|---|---|---|
| **Público** | `https://htmlbox.app/s/{shareId}` | Cualquiera con el link. SDK corre con token **read-only** de esa DB embebido al servir el HTML. Landing pages, reportes públicos |
| **Privado** | `https://{tenant}.htmlbox.app/{boxSlug}` | Requiere sesión + rol ≥ Lector. Escrituras requieren Editor+. ERPs, CRMs, herramientas internas |

### Aislamiento

- El HTML de usuario (potencialmente no confiable) se sirve en el dominio de
  runtime (`*.htmlbox.app`), **nunca** en `portal.htmlbox.app` ni
  `controlplane.htmlbox.app` — el sandbox del navegador aísla cookies/storage
  por origen.
...
Endpoint en el runtime: `https://{tenant}.htmlbox.app/{boxSlug}/mcp`
