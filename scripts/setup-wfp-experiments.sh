...
echo "  6. → Crear R2 bucket para storage de boxes:"
echo "         npx wrangler r2 bucket create htmlbox-studio-boxes"
echo "  7. → Agregar el binding al wrangler.jsonc del launcher:"
echo "         \"r2_buckets\": [{ \"binding\": \"STUDIO_R2\", \"bucket_name\": \"htmlbox-studio-boxes\" }]"
echo "  8. → npm run build -w @htmlbox/sivostudio"
echo "         (genera box-template/editors/app-studio.html.txt desde Chats/.../repl-svelte/"
echo "          + bundlea dist/box-worker.mjs)"
echo "  9. → npm run dev -w @htmlbox/sivostudio"
