...
Depende de `01-htmlbox-spec-tablas-reales.md` (columnas SQL reales, `PRAGMA table_info` como fuente de verdad) y reemplaza el flujo de una sola pasada de `03-htmlbox-spec-ai-analyze-robusto.md` §2 por un loop de tool-calling. El resto de esa spec (clasificación de `app_type`, `created_by`, UI del modal) no cambia.
...
Tools (ejecutan contra la Turso del box vía `getBoxClient`, reusando lo de `01-htmlbox-spec-tablas-reales.md`):
...
- No se resuelve recrear una tabla para agregarle UNIQUE/FK después de creada (mismo punto que ya quedó fuera de alcance en `01-htmlbox-spec-tablas-reales.md` §5).
