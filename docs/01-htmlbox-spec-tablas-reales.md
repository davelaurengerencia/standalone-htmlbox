...
Decisión de David: reemplazar el modelo actual de blob JSON por columnas SQL reales en TODA la capa de datos de un box (no solo lo que crea la IA) — manual, CSV upload, bulk-create y el análisis con IA comparten el mismo storage hoy (`packages/runtime/src/lib/dataApi.js`), así que el cambio es transversal. Esta spec **reemplaza** la sección §3 ("promover columnas") de `03-htmlbox-spec-ai-analyze-robusto.md` — esa spec pasa a apoyarse en esta.
...
## 6. Impacto en `03-htmlbox-spec-ai-analyze-robusto.md`
